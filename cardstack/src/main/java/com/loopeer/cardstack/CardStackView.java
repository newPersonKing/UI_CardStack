...
public class CardStackView extends ViewGroup implements ScrollDelegate {
    ...
    private int mTotalLength; /*所有子view的高度之和*/
    private int mOverlapGaps; /*可以理解为卡片重叠的部分*/
    ...
    private int mShowHeight; /*自己允许展示的最大高度*/
    ...
    private void measureChild(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            Log.i("ccccccccccc","mHeaderHeight=="+lp.mHeaderHeight);
            Log.i("ccccccccccc","getMeasuredHeight=="+child.getMeasuredHeight());
        ...
        Log.i("ccccccccccccc","childCount==="+getChildCount());
        Log.i("ccccccccccccc","mTotalLength==="+mTotalLength);
        Log.i("ccccccccccccc","mOverlapGaps==="+mOverlapGaps);
        mTotalLength += mOverlapGaps * 2;
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if ((mIsBeingDragged = !mScroller.isFinished())) {
                    final ViewParent parent = getParent();
                    if (parent != null) {
                        /*requestDisallowInterceptTouchEvent true 代表告诉parent 这个我自己来处理*/
                        parent.requestDisallowInterceptTouchEvent(true);
                    }
                }
